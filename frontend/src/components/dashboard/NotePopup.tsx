 ...
 *
 * Used in both the Value Screener (note indicator dot) and the Watchlist table.
...
import { X, StickyNote, Pencil, Check } from 'lucide-react'
import type { WatchlistNote } from '@/lib/types'
import { useUpdateNoteById } from '@/hooks/useWatchlist'


interface NotePopupProps {
  /** All notes for this ticker (may span multiple users/watchlist items) */
  notes: WatchlistNote[]
  ticker: string
...
export function NotePopup({ notes, ticker, anchorRect, onClose }: NotePopupProps) {
  ...
  // Track which note ID is being edited (null = none)
  ...
  const startEdit = useCallback((note: WatchlistNote) => {
  ...
  return createPortal(
    ...
    >
      ...
      <div className="max-h-64 overflow-auto p-3 space-y-3">
        {notes.length === 0 ? (
          <p className="text-xs text-[var(--muted-foreground)] italic">No notes yet.</p>
        ) : (
          notes.map((note) => (
              ...
              <div className="flex items-center justify-between mb-1">
                ...
                {editingId === note.id ? (
                  <div className="flex items-center gap-1">
                    <button onClick={saveEdit} className="p-0.5 text-gain hover:text-gain/80" title="Save">
                      <Check className="w-3 h-3" />
                    </button>
                    <button onClick={cancelEdit} className="p-0.5 text-[var(--muted-foreground)] hover:text-loss" title="Cancel">
                      <X className="w-3 h-3" />
                    </button>
                  </div>
                ) : (
...
// ============================================================================
// NoteIndicator — small dot/button shown in the screener or watchlist row.
// Clicking it opens the NotePopup.
// ============================================================================
  ...
  return (
    <>
      <button
        ...
        title={`${notes.length} note${notes.length > 1 ? 's' : ''} on ${ticker}`}
      />
      {open && (
        <NotePopup
          notes={notes}
